...
open import Category hiding (refl)
open import CatPrelude.Function using (id)
...
_∙ᵣ_ {A} {B} {C} RBC RAB =  λ axc → Σ B λ b → RAB (proj₁ axc , b) ∧ RBC (b , proj₂ axc)


Idᵣ : (A : Set) → R A A
Idᵣ _ = λ AxB → proj₁ AxB ≡ proj₂ AxB
...
assoc-∙ : {A B C D : Set} (f : R A B) (g : R B C) (h : R C D)
        → (h ∙ᵣ (g ∙ᵣ f)) ≈ᵣ ((h ∙ᵣ g) ∙ᵣ f)
